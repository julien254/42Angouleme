...
/*   Updated: 2022/07/02 01:56:29 by julien           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

void	check_fd(int fd, t_listgnl *filed)
{
	int	i;
	int	j;
...
char	*ft_save_residu(t_listgnl *filed, char *ligne)
{
	int	i;
	int	j;
	...
	if (j == 0)
		ft_memset_gnl(filed->save.residu[filed->save.index_currentfd]);
	ft_memset_gnl(filed->residu);
	return (ligne);
}

char	*ft_split_and_save(t_listgnl *filed)
{
	int		i;
	int		j;
	char	*ligne;

	ligne = (char *)malloc(end_line_gnl(filed->residu) + 2);
	if (!ligne)
		return (ligne);
	i = 0;
...
char	*ft_readbuf(t_listgnl *filed)
{
	char	*str_read;
	int		reader;
...
{
	static t_listgnl	filed = {0, {0}, {{0}, {""}, {0}, 0}};
	char				*str_read;
	char				*ligne;
